const KIE_KEYS = [
  ...
  process.env.KIE_API_KEY_12,
  process.env.KIE_API_KEY_13,
  process.env.KIE_API_KEY_14,
  process.env.KIE_API_KEY_15
];
...
export const generateSong = async () => {
  try {
    ...
    for (let i = 0; i < KIE_KEYS.length; i++) {
      ...
      // If success
      if (data?.data?.taskId) {
        console.log("🎧 Success using key", i + 1,`task id :- ${data.data.taskId}`);
