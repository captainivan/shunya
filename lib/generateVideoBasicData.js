...
export const generateVdieoBasicData = async () => {
    try {
        const genre = songGenre[Math.floor(Math.random() * songGenre.length)].genere;
        const style = songGenre[Math.floor(Math.random() * songGenre.length)].style;
...
TITLE
- Emotional, human-sounding
- No hashtags
- End with channel name

Format:
{emotion-based title} | Shunya

Examples:
- A song for quiet nights | Shunya  
- This song feels like loneliness | Shunya  
- For people who think too much | Shunya  

---

DESCRIPTION
- 1–5 calm, emotional lines (natural language)
- Line breaks are ALLOWED
- Then maximum 5 hashtags (space-separated)

Format:
{short emotional lines} #nightvibes #calmsong #emotionalmusic #headphones #shunya
...
- No tag stuffing  
- Avoid excessive symbols (|, ★, !!!, emojis)
- Simple punctuation is allowed
- No special characters like backslash or escape character in title and desc

=========== LYRICAL STRUCTURE ==================
Lyrics MUST include all sections:
${style}

Each section MUST clearly express its identity by drawing from
ONE OR MORE of the following expressive layers:

- Scene or emotional context
  (visual imagery, inner state, symbolic or situational setting)

- Instrumental or sound-design cues
  (instruments, textures, rhythms, or sonic movement)

- Vocal presence or intentional absence
  (delivery style, texture, spoken, sung, whispered, or silence)

- Atmosphere, space, or environmental motion
  (air, distance, echoes, pauses, stillness, movement)

Not every section needs every layer.
Minimalism, silence, or absence are valid artistic choices.

At least ONE section in the song MUST intentionally violate expectation
(e.g., silence where sound is expected, vocals where none existed,
or a sudden removal of energy).
================================================
